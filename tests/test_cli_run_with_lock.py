...
def test_cli_vmstat(lock_server: ServerThread, lock_name: str):
...
def test_cli_vmstat_with_shell(lock_server: ServerThread, lock_name: str):
    args = ('python', '-m', 'tcpnetlock.cli.run_with_lock',
            '--host=localhost',
            '--port={port}'.format(port=lock_server.port),
            '--lock-name={lock_name}'.format(lock_name=lock_name),
...
def test_cli_vmstat_with_shell_and_pipe(lock_server: ServerThread, lock_name: str):
    args = ('python', '-m', 'tcpnetlock.cli.run_with_lock',
            '--host=localhost',
            '--port={port}'.format(port=lock_server.port),
            '--lock-name={lock_name}'.format(lock_name=lock_name),
...
def test_cli_fails_log_not_granted(lock_server: ServerThread, lock_name: str):
